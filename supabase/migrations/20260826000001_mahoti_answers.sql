-- mahoti_answers — one row per sitting of a דיון מהותי paper: who sat it,
-- which letters they chose, and what that scored.
--
-- WHY A NEW TABLE, AND NOT user_id ON mahoti_questions.
-- mahoti_questions is the CONTENT: one row per generated paper, the same paper
...
--
-- NAMING: `mahoti_answers`, plural and lower-case, to match open_question_answers
...
--                   against auth.uid().
--   * question_id   uuid -> mahoti_questions. WHICH paper was sat. Without it
...
--
-- WHO WRITES THE SCORE. The answer key lives in mahoti_questions.questions ->
...
CREATE TABLE IF NOT EXISTS public.mahoti_answers (
  answer_id     uuid              PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id       uuid              NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  question_id   uuid              NOT NULL REFERENCES public.mahoti_questions(question_id) ON DELETE RESTRICT,
  ...
  CONSTRAINT mahoti_answers_score_range
...
CREATE INDEX IF NOT EXISTS idx_mahoti_answers_user_created_at
  ON public.mahoti_answers (user_id, created_at DESC);

-- Per-paper statistics: "how did everyone do on this paper".
CREATE INDEX IF NOT EXISTS idx_mahoti_answers_question
  ON public.mahoti_answers (question_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_mahoti_answers_user_question_attempts
  ON public.mahoti_answers (user_id, question_id, attempts DESC);
...
CREATE OR REPLACE FUNCTION public.set_mahoti_answer_attempts()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
DROP TRIGGER IF EXISTS trg_mahoti_answers_attempts
  ON public.mahoti_answers;

CREATE TRIGGER trg_mahoti_answers_attempts
  BEFORE INSERT ON public.mahoti_answers
  FOR EACH ROW
  EXECUTE FUNCTION public.set_mahoti_answer_attempts();

-- ---------------------------------------------------------------------------
-- RLS
-- ---------------------------------------------------------------------------
ALTER TABLE public.mahoti_answers ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY mahoti_answers_students_select_own
  ON public.mahoti_answers FOR SELECT TO authenticated
  USING (user_id = (SELECT auth.uid()));

-- Admins read everything, for the per-paper and per-cohort statistics.
CREATE POLICY mahoti_answers_admins_select
  ON public.mahoti_answers FOR SELECT TO authenticated
...
GRANT SELECT ON public.mahoti_answers TO authenticated;

COMMENT ON TABLE public.mahoti_answers
  IS 'One row per sitting of a mahoti paper: the letters chosen, how they were marked, and the resulting score. Written server-side with the service role; students may only read their own rows.';
COMMENT ON COLUMN public.mahoti_answers.answer_score
  IS 'Percentage correct out of the paper''s total questions (30/40 -> 75). Computed server-side against the answer key.';
COMMENT ON COLUMN public.mahoti_answers.attempts
