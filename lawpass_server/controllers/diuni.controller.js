...
// דין דיוני — filing and scoring a sitting of a generated paper.
//
...
// (lib/db/diuni.ts strips it), so the client genuinely cannot mark itself —
// and diuni_answers has no INSERT policy for `authenticated`, so it cannot
// file a score it made up either. A submission is a list of letters; what that
// is worth is decided on this side of the wire.

const db = require("../db/diuni");
...
/**
 * POST /api/diuni/questions/:id/attempts — file this sitting.
 *
 ...
 * student's own history for this paper (see 20260831000004), so it cannot
...
async function submitAttempt(req, res) {
  ...
  if (!answerKey) {
    console.info(
      `[diuni] attempt MISS user=${req.user.id} question=${questionId}`
    );
  ...
  console.info(
    `[diuni] attempt OK user=${req.user.id} question=${questionId} ` +
...
/**
 * GET /api/diuni/attempts[?question_id=…] — this student's own sittings.
 *
 * Always scoped to req.user.id. The admin client is used for the read (see
 * db/diuni.js), so the scoping is this line rather than RLS — passing a
...
async function listAttempts(req, res) {
  ...
  console.info(
    `[diuni] attempts OK user=${req.user.id} count=${attempts.length}` +
