...
| Block with stripped question-number marker that is a content duplicate of the previous block (Sharon's 2019-S-S file 1-21 position 5 = exact dup of Q05 with "5." prefix removed) | parser → normalize.py | normalize.py treats `marker == 0` as a skip (recorded as a duplicate) — safer than guessing a slot and pushing the legitimate next question into the duplicate-skip path |
| `references_list: null` / `concepts_and_skills: null` in source JSON (Sharon's 2022-W-S Q17) | per-question array fields | generator emits `'[]'::jsonb` instead of `NULL::jsonb` for these array columns via `sql_jsonb_array()` — the DB columns are `NOT NULL DEFAULT '[]'::jsonb`, so a stray NULL would fail the insert |
