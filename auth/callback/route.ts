 ...
 *
 * Flow:
 *  - User clicks the Google button → /auth/google route handler redirects to Google.
...
export async function GET(request: Request) {
  ...
  if (exchangeError) {
    // Real production error — the PKCE code exchange failed. Worth keeping
    // as console.error so it surfaces in Vercel runtime logs.
