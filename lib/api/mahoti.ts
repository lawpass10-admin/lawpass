...
/**
 * דיון מהותי — client wrapper over the Express API.
 *
 * Only the SITTING goes through here. The paper itself — questions, notebook,
 * review — is still read by the Next.js Server Components in lib/db/mahoti.ts,
 * which reach `mahoti_questions` through the service-role client because the
 ...
 * next `attempt_number`.
 */
export async function submitMahotiAttempt(
  questionId: string,
  given: MahotiGivenAnswer[]
): Promise<Result<MahotiAttempt>> {
  if (!apiEnabled()) return { ok: false, error: DISABLED_ERROR };
  try {
    const body = await apiPostJson(
      `/api/mahoti/questions/${encodeURIComponent(questionId)}/attempts`,
      { given },
      { auth: true }
    );
    if (body.ok === true) {
      return { ok: true, data: body.attempt as MahotiAttempt };
    }
    return failed(body);
  } catch {
 ...
 */
export async function fetchMahotiAttempts(
  questionId?: string
): Promise<Result<MahotiAttempt[]>> {
  if (!apiEnabled()) return { ok: false, error: DISABLED_ERROR };
  try {
    const path = questionId
      ? `/api/mahoti/attempts?question_id=${encodeURIComponent(questionId)}`
      : "/api/mahoti/attempts";
    const body = await apiGetJson(path, { auth: true });
    if (body.ok === true) {
      return { ok: true, data: (body.attempts ?? []) as MahotiAttempt[] };
    }
    return failed(body);
  } catch {
