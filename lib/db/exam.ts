...
// Slice 21 — the full-row SELECT constants + the two batched
// resolvers + the Learning360 types moved to lib/db/learning360.ts
// so the practice-summary page can reuse them. Imported back here
// so the rest of exam.ts (notably `getQuestionForExamPosition`'s
// per-position SELECT) keeps working, and re-exported so existing
// callers + tests that import these names from "@/lib/db/exam"
...
export type { Learning360Payload };
...
// Slice 17 B-2 — Learning360Payload moved to lib/db/learning360.ts
// (Slice 21). Re-exported above for back-compat with callers that
// import it from "@/lib/db/exam".
...
// Slice 21 — `resolveChoicesForList`, `resolveLearning360ForList`,
// and the `Learning360FieldsOnly` intermediate type moved to
// `lib/db/learning360.ts` so the practice-summary page can reuse
// them. Imported + re-exported at the top of this module so existing
// callers + tests that import them from "@/lib/db/exam" keep working.
